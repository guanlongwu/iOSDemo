...
static BOOL aspect_isCompatibleBlockSignature(NSMethodSignature *blockSignature, id object, SEL selector, NSError **error) {
    ...
    if (blockSignature.numberOfArguments > methodSignature.numberOfArguments) {
        ...
        if (blockSignature.numberOfArguments > 1) {
            // Aspects要求，如果block有参数，第一个参数必须是 id<AspectInfo>对象，所以参数签名是  @
...
static void aspect_swizzleForwardInvocation(Class klass) {
    ...
    /**
     class_replaceMethod 此函数以两种不同的方式运行：
     如果名称标识的方法尚不存在，则添加它，就像调用了 class_addMethod 一样。由 types 指定的类型编码按给定的方式使用。
     如果名称标识的方法确实存在，则替换其 IMP，就好像调用了 method_setImplementation 一样。类型指定的类型编码被忽略。
     */
