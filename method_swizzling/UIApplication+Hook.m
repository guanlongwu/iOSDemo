...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        KMSwizzleMethod([self class],
                        ...
                        [self class],
                        @selector(gl_sendAction:to:from:forEvent:));
        KMSwizzleMethod([self class],
...
- (BOOL)gl_sendAction:(SEL)action to:(id)target from:(id)sender forEvent:(UIEvent *)event
{
//    NSLog(@"%s", __func__);
    return [self gl_sendAction:action to:target from:sender forEvent:event];
}
